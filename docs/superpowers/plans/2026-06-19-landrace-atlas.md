...
After B1/C1 verify rendering: generate `data/writeups/<id>.md` for every strain in `landraces.json`, in batches (group by continent/region), each batch a subagent following `docs/writeup-generation-guide.md` exactly. Validate after: every id has a file; every file starts with the disclaimer and contains the 8 section headings; no `http`/`www` URLs appear in any generated file (proves no fabricated links) except none — assert zero URLs. Commit per batch.
