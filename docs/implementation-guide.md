...
| `graticule.js` | `createGraticule(map)` — the **Lat/Long Lines** overlay: a zoom-adaptive coordinate graticule as `L.geoJSON` in its own pane, generated at runtime (no data file), equator/prime meridian stronger. Independent toggle at the top of the geometry stack (above Labels), not gated by Labels. Pure `graticuleStep`/`graticuleFeatures` unit-tested in `graticule.test.mjs`. |
