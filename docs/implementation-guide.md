...
| `labels.js` | `createLabels(map, data)` — zoom-aware text labels in four toggle groups (`place`/`states`/`rivers`/`terrain`), drawn in a pane below the markers. Lake names live in `place` (Labels toggle); the `terrain` group holds range/peak names + desert/plateau/basin/delta names. On each map settle a `declutter()` pass resolves state-vs-city label overlaps: it flips the city's text to the left of its dot when that side is clear, else nudges the state label vertically to clear the city. Exports the pure helpers (`*MinZoom`, `peakSizeTier`, `reliefMaxLevel`, `rectsOverlap`) unit-tested in `labels.test.mjs`. |
...
| `graticule.js` | `createGraticule(map)` — the **Latitude/Longitude** overlay: a zoom-adaptive coordinate graticule as `L.geoJSON` in its own pane, generated at runtime (no data file), equator/prime meridian stronger. When zoomed out (≤ z3) it also draws degree labels on a small canvas — latitudes at the right end of the parallels, longitudes along both the bottom and top edges. Independent toggle at the top of the geometry stack (above Labels), not gated by Labels. Pure `graticuleStep`/`graticuleFeatures`/`fmtLat`/`fmtLng` unit-tested in `graticule.test.mjs`. |
| `panes.js` | `PANE_Z` — the single source of truth for overlay pane stacking (z-index). Every pane-creating module reads its z-index here so the order can't drift. Load-bearing invariant (in `panes.test.mjs`): the heat/climate **data tints sit below** the reference geometry (lakes, relief, borders, rivers, graticule) so those stay visible over an active heat map; labels and markers are always on top. |
...
| `climate.js` | `createClimate(map)` — the **climate** heat maps, one at a time. **Growing season temperature** and **growing season rainfall** bilinearly sample the pre-computed land-only grid `data/geo/climate.json` (lazy-loaded) onto a **canvas**, coverage-scaled alpha feathering the coastline; **growing season daylight** (land-only latitude bands rounded to whole hours over a dark→gold ramp, with merged sun-badge labels) and **growing season solar energy** (land-only, smooth, clear-sky surface insolation) are valued by latitude from solar geometry (`growDaylight` / `growInsolation`, the latter integrating the air-mass-attenuated beam across the day), using the grid only as a land mask; daylight and solar share the dark→gold ramp (brightest = most). Per-metric ramps (no green) + unit legends. Pure `rampColor`/`norm`/`legendGradient`/`growDaylight`/`growInsolation`/`METRICS` unit-tested in `climate.test.mjs`. The flowering + climate maps form one **mutually-exclusive** toggle group (`setHeat`/`HEATMAPS` in `app.js`, second toggle bar + a **Heat Maps** ☰-submenu, key `cla-heatmap`). |
