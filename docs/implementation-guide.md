...
| `graticule.js` | `createGraticule(map)` — the **Latitude/Longitude** overlay: a zoom-adaptive coordinate graticule as `L.geoJSON` in its own pane, generated at runtime (no data file), equator/prime meridian stronger. When zoomed out (≤ z3) it also draws degree labels on a small canvas — latitudes down the right edge, longitudes along the bottom. Independent toggle at the top of the geometry stack (above Labels), not gated by Labels. Pure `graticuleStep`/`graticuleFeatures`/`fmtLat`/`fmtLng` unit-tested in `graticule.test.mjs`. |
