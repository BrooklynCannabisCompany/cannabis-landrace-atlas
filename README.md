...
- **Climate heat-map data** (`data/geo/climate.json` — growing season temperature and rainfall) is derived from [NASA POWER](https://power.larc.nasa.gov/) climatology (MERRA-2, 2001–2020 monthly means), which is in the public domain. It is generated once, offline, by `data/build/gen-climate.mjs` (needs network), like the Natural Earth files above, and is not covered by the dataset's CC BY-SA license. The growing season daylight map has no data source of its own — its values are computed from latitude in the browser; it only reuses the temperature grid as a land mask.
