...
- **Map overlays:** top-left toggles (mirrored in the ☰ menu) add optional, zoom-aware cartography. **Latitude/Longitude** overlays an independent coordinate graticule (spacing adapts to zoom; equator and prime meridian drawn stronger; degree labels on the right/bottom edges when zoomed out). **Labels** is the master text switch — turn it off and no names show; turn it on and you get names for whatever other layers are enabled (plus base country/city/ocean/lake names). **States & Provinces** (admin-1 borders), **Rivers** (lines), and **Terrain** (mountain ranges as a triangle-relief field with named peaks, plus deserts/plateaus/basins/deltas and a subtle desert tint) each draw their geometry on their own; their names appear when both that toggle and Labels are on. Water (oceans, lakes, rivers) is aqua; major lake outlines are always drawn. All toggles are off by default and remembered between visits; the larger layers load on first use.
- **Heat maps:** a second toggle group, grouped under a **Heat Maps** submenu in the ☰ menu, where only one is active at a time (or none) and each tints the map by a single variable. **Flowering** interpolates each variety's flowering time from its true coordinates on an absolute grower scale (≤8w blue → 16w+ red). Two **land-only climate** maps — **Growing Season Temperature** and **Growing Season Rainfall** — render a pre-computed global grid (`data/geo/climate.json`) built once from NASA POWER climatology; the "growing season" is each location's six warmest months, so it is hemisphere-correct (polar regions are shown at coarser resolution). Two more maps are computed purely from latitude (solar geometry, no data): **Growing Season Daylight** as hour-quantized latitude bands, and **Growing Season Solar Energy** — clear-sky surface insolation (kWh/m²·day), which folds the sun's angle into the day length and so peaks in the subtropics rather than at the equator or the poles. Both share a dark→gold ramp where brightest = most. Each map has its own legend with units; green is reserved for the leaf pins, which always draw on top.
...
- **Climate heat-map data** (`data/geo/climate.json` — growing season temperature and rainfall) is derived from [NASA POWER](https://power.larc.nasa.gov/) climatology (MERRA-2, 2001–2020 monthly means), which is in the public domain. It is generated once, offline, by `data/build/gen-climate.mjs` (needs network), like the Natural Earth files above, and is not covered by the dataset's CC BY-SA license. The growing season daylight and solar-energy maps have no data source of their own — their values are computed from latitude (solar geometry) in the browser; they only reuse the temperature grid as a land mask.
