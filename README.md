...
- **Map overlays:** top-left toggles (mirrored in the ☰ menu) add optional, zoom-aware cartography — **Labels** (country/city/ocean names), **States & Provinces** (admin-1 borders + labels for landrace-relevant countries), **Rivers**, and **Mountains** (ranges drawn as a triangle-relief field, with named peaks). Major lakes are always shown. All are off by default and remembered between visits; the larger layers load on first use.
