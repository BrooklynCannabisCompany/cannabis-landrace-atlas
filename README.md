...
- **Climate heat-map data** (`data/geo/climate.json` — summer temperature and growing season rainfall) is derived from [NASA POWER](https://power.larc.nasa.gov/) climatology (MERRA-2, 2001–2020 monthly means), which is in the public domain. It is generated once, offline, by `data/build/gen-climate.mjs` (needs network), like the Natural Earth files above, and is not covered by the dataset's CC BY-SA license. The growing season daylight map needs no data — it is computed from latitude in the browser.
