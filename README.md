...
- **Map overlays:** top-left toggles (mirrored in the ☰ menu) add optional, zoom-aware cartography — **Labels** (country/city/ocean/lake names), **States & Provinces** (admin-1 borders + labels for landrace-relevant countries), **Rivers**, and **Mountains** (ranges drawn as a triangle-relief field, with named peaks). Major lake outlines are always drawn on the basemap; their names appear with the Labels toggle. All toggles are off by default and remembered between visits; the larger layers load on first use.
