...
- **Map overlays:** top-left toggles (mirrored in the ☰ menu) add optional, zoom-aware cartography — **Labels** (country/city/ocean/lake names), **States & Provinces** (admin-1 borders + labels for landrace-relevant countries), **Rivers**, and **Terrain** (mountain ranges as a triangle-relief field with named peaks, plus deserts/plateaus/basins/deltas and a subtle desert tint). Water (oceans, lakes, rivers) is aqua; major lake outlines are always drawn on the basemap and their names appear with the Labels toggle. All toggles are off by default and remembered between visits; the larger layers load on first use.
